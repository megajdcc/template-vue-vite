            ...
            <b-row>
               <b-col cols="12" v-if="evento.imagenes.length">
                     <b-carousel :id="`carousel-${evento.id}`" indicators controls background="#ababab" :intervals="3000">
                        <b-carousel-slide v-for="(imagen, e) in evento.imagenes.filter(val => !val.is_thumb)" :key="e" img-width="320px" img-height="auto"
                           :img-src="`${$root.api_uri}/storage/eventos/imagenes/${imagen.imagen}`"
